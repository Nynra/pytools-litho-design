...
def alignment_mark(layers: List[int] = [1, 2, 3, 4]) -> gf.Component:
    ...
    def create_marker(layer1, layer2):
        ...
        CROSS = gf.Component()
        cross = CROSS << gf.components.cross(length=190, width=20, layer=layer1)
        rect = gf.components.rectangle((65, 65), layer=layer2)
        # window = CROSS.add_array(rect, 2, 2, (125, 125))
        window = gf.components.containers.array(rect, 2, 2, 125, 125)
        ...
        # text
        TEXT = gf.Component()
        text1 = TEXT << gf.components.text(str(layer2), size=50, layer={layer1, layer2})
        text1.move(text1.center, (220, 200))
        text2 = TEXT << gf.components.text(
            f"{layer2} ON {layer1}", size=10, layer=layer2
        )
    ...
    for i, layer1 in enumerate(layers):
        ...
        if n != 0:
            for j, layer2 in enumerate(layers[-n:]):
                ...
                ALIGN << MARK
            text = gf.components.text(str(layer1), size=160, layer=layer1)
...
def resolution_test(
    ...
    def create_3L(res=1):
        ...
        def create_L(w, spacing):
            ...
            bar = gf.components.rectangle((min(100 * w, 100), w))
            bars = gf.Component()
            # bars.add_array(bar, 1, 5, spacing=(0, spacing))
            bars = gf.components.containers.array(bar, 1, 5, 0, spacing)
            v_bars = L << bars
            h_bars = L << bars
            h_bars.rotate(90)

            # Alight all the components in L to the min x axis
            L.align("all", "xmin")
        ...
        text = LLL << gf.components.text(str(res), size=20)
        text.move(
    ...
    def create_waffle(res=1):
        ...
        WAFFLE = gf.Component()
        W = gf.components.rectangle(size=(res * 80, res * 80))

        pattern = [(res * x, res * 80) for x in [2, 1, 1, 2, 3, 5, 8, 13, 21, 15]]
        WOut = gf.components.gridsweep(
            function=gf.components.rectangle,
        ...
        WAFFLE << W1
        WAFFLE << W2
        text = WAFFLE << gf.components.text(str(res), size=20)
        text.move(
...
def vdp(l: float = 400, w: float = 40, layer: int = 1) -> gf.Component:
    ...
    polygon = gf.components.polygon_ports(xpts=xpts, ypts=ypts, layer=layer)
    VDP << polygon
    VDP.flatten()
