...
spiral_racetrack = partial(gf.components.spiral_racetrack, cross_section="asic_routing")
...
def spiral_racetrack_fixed_length(
    ...
    # if isinstance(straight, str):
    #     straight = gf.get_component(straight, cross_section=xs)
    ...
    # To fix the type error bug (gdsfactory doesnt accept np.XXX types)
    if min_spacing + cross_section.width <= 1:
        raise ValueError(
            "min_spacing must be more than 1, below causes a calculation error"
        )
    min_spacing = float(min_spacing + cross_section.width)
    spacings = [min_spacing for _ in range(n_straight_sections // 2)]
